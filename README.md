...
- Run in Terminal: `docker pull exsilium/unifi-video:v3.9.0`

```
`$ docker images
REPOSITORY             TAG                 IMAGE ID            CREATED             SIZE
exsilium/unifi-video   v3.9.0              1cbeb1e369da        44 minutes ago      869.9 MB
```

- (Optional) Download\Save `run.sh` from [here](https://raw.githubusercontent.com/exsilium/docker-unifi-video/v3.9.0/run.sh)
...
**NB!** Note that, when upgrading to v3.9.0, a new port 7442 was added in v3.8 for secure camera communications. Make sure docker maps this new port when starting and that you handle any additional routing settings you may have to that port.

- Stop the running container
- Backup your Host Data Volumes (`~/Applications/unifi-video`)
- Pull the latest image `docker pull exsilium/unifi-video:v3.9.0`
...
- Update the `run.sh` to reflect the new version (v3.9.0)
