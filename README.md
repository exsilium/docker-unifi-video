...
- Run in Terminal: `docker pull exsilium/unifi-video:v3.10.5`

```
`$ docker images
REPOSITORY             TAG                 IMAGE ID            CREATED             SIZE
exsilium/unifi-video   v3.10.5            1cbeb1e369da        44 minutes ago      869.9 MB
```

- (Optional) Download\Save `run.sh` from [here](https://raw.githubusercontent.com/exsilium/docker-unifi-video/v3.10.5/run.sh)
...
- Use the Unifi-Video web interface to launch an upgrade, this upgrade will not finish successfully, but will upgrade the database collections to the new `v3.10.5` format
...
- Start the version v3.10.5 container
...
**NB!** Note that, when upgrading to v3.10.5 from previous minor version, you first need to upgrade to v3.9.12.
